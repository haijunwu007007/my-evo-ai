...
# ── 引导用户输入 ──
logger.info("=" * 60))
logger.info("  腾讯云安全组配置 — 开放 8765 端口"))
logger.info("=" * 60))
logger.info())
logger.info("▶ 请登录 https://console.cloud.tencent.com/cam/capi"))
logger.info("  创建一个 API 密钥（或使用已有密钥）"))
logger.info())
...
if not secret_id or not secret_key:
    logger.info("❌ 需要 SecretId 和 SecretKey 才能操作"))
    sys.exit(1)
...
try:
    ...
    logger.info("正在安装 tencentcloud-sdk-python..."))
...
def get_instance_security_groups():
    ...
    try:
        ...
        if instances.get("TotalCount", 0) == 0:
            ...
            instances = json.loads(resp.to_json_string())
        if instances.get("TotalCount", 0) == 0:
            logger.info(f"❌ 未找到 IP {INSTANCE_IP} 对应的实例，请手动在控制台配置安全组"))
            return None, None
        instance = instances["InstanceSet"][0]
        sg_ids = [sg["SecurityGroupId"] for sg in instance.get("SecurityGroupIds", [])]
        logger.info(f"✅ 找到实例: {instance.get('InstanceName', 'unknown')} ({instance.get('InstanceId', '')})"))
        logger.info(f"  关联安全组: {sg_ids}"))
        return instance, sg_ids
    except TencentCloudSDKException as e:
        logger.info(f"⚠ 查询实例失败: {e}"))
        logger.info("  改用安全组名称搜索..."))
        return None, None
    ...
    try:
        ...
        for sg in sgs.get("SecurityGroupSet", []):
            if sg.get("SecurityGroupName", "").startswith("default"):
                logger.info(f"  找到默认安全组: {sg['SecurityGroupId']} ({sg.get('SecurityGroupName', '')})"))
                return sg["SecurityGroupId"]
        if sgs.get("SecurityGroupSet"):
            sg = sgs["SecurityGroupSet"][0]
            logger.info(f"  使用安全组: {sg['SecurityGroupId']} ({sg.get('SecurityGroupName', '')})"))
            return sg["SecurityGroupId"]
    except Exception as e:
        logger.info(f"⚠ 查询安全组失败: {e}"))
    return None
    ...
    try:
        ...
        for p in policies.get("SecurityGroupPolicySet", {}).get("Ingress", []):
            if p.get("Port") == "8765" and p.get("Protocol") == "TCP":
                logger.info(f"✅ 8765 端口规则已存在，无需重复添加"))
                return True
        ...
        logger.info(f"✅ 成功添加 8765 端口入站规则到安全组 {sg_id}"))
        return True
    except TencentCloudSDKException as e:
        logger.info(f"❌ 添加规则失败: {e}"))
        return False
...
def main():
    ...
    if not sg_id and sg_ids:
        sg_id = sg_ids[0]
    if not sg_id:
        logger.info("❌ 无法确定安全组，请手动在控制台配置"))
        logger.info("  登录 https://console.cloud.tencent.com/cvm/securitygroup"))
        sys.exit(1)
    logger.info(f"  目标安全组: {sg_id}"))
    ...
    if success:
        ...
        logger.info())
        logger.info("  现在尝试访问："))
        logger.info(f"  → http://{INSTANCE_IP}:8765/"))
        logger.info())
        logger.info("  仍无法访问的话，登录控制台确认规则："))
        logger.info("  https://console.cloud.tencent.com/cvm/securitygroup"))
    else:
        logger.info("\n  ⚠ 自动配置失败，请手动操作："))
        logger.info("  https://console.cloud.tencent.com/cvm/securitygroup"))
        logger.info("  入站规则 → 添加规则 → TCP:8765 → 0.0.0.0/0 → 允许"))
