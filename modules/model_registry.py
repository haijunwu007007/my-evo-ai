...
class ModelRegistryModule:
    ...
    def register_model(self, params: dict) -> dict:
        ...
        self._audit_log.append({"action": "register", "model": name, "timestamp": datetime.utcnow().isoformat()})
    ...
    def deprecate_model(self, params: dict) -> dict:
        ...
        self._models[name]["updated_at"] = datetime.utcnow().isoformat()
        self._stats["active_models"] = sum(1 for m in self._models.values() if m.get("status") == ModelStatus.ACTIVE)
        self._audit_log.append({"action": "deprecate", "model": name, "timestamp": datetime.utcnow().isoformat()})
    ...
    def delete_model(self, params: dict) -> dict:
        ...
        self._stats["active_models"] = sum(1 for m in self._models.values() if m.get("status") == ModelStatus.ACTIVE)
        self._audit_log.append({"action": "delete", "model": name, "timestamp": datetime.utcnow().isoformat()})
