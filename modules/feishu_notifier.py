# -*- coding: utf-8 -*-
"""AUTO-EVO-AI v7.0 - 飞书通知器（A级）"""
__module_meta__ = {"id":"feishu-notifier","name":"Feishu Notifier","version":"v7.1","group":"notify","grade":"A",
...
class FeishuNotifier(CircuitBreakerMixin,RateLimiterMixin,EnterpriseModule):
    MODULE_ID="feishu-notifier";MODULE_NAME="飞书通知";VERSION="v7.1";MODULE_LEVEL="A"
    ...
    def _dispatch(self,p):
        ...
        if a=="send_batch":msgs=p.get("messages",[]);sent=[];
        for m in msgs:self._history.append({"type":"batch","content":m.get("text",""),"timestamp":time.time()});sent.append({"text":m.get("text",""),"sent":True})
