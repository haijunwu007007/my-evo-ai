...
// ── 搜索缓存 ──
var _searchCache={};var _searchCacheTTL=120000;
function _cachedSearch(url,cb){var now=Date.now();var cached=_searchCache[url];if(cached&&(now-cached.ts<_searchCacheTTL)){cb(cached.data);return}fetch(url).then(function(r){if(r.status!==200)return r.text().then(function(t){cb(null)});return r.json().then(function(d){_searchCache[url]={data:d,ts:now};cb(d)})}).catch(function(){cb(null)})}
