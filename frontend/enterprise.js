        ...
        const DEFAULT_MODULES = [
            ...
            { id: 'elasticsearch-search', name: 'Elasticsearch', group: '数据库存储', icon: '🔍', desc: '全文搜索，日志分析', source: 'Elastic', stars: '核心', tags: ['搜索', '日志'], color: '#005571' },
            ...
            { id: 'agent-orchestrator', name: '主编排器', group: '系统大脑', icon: '🧠', desc: '系统智能大脑：自然语言理解→任务拆解→62模块自动调度→执行→学习→改进，一句话完成复杂任务', source: 'AUTO-EVO-AI', stars: 'Core', tags: ['编排', '调度', '大脑', '自动化'], color: '#EF4444', priority: 'critical' },
        ...
        // AUTO-EVO-AI V0.1 - 后端564模块均已注册，不再使用静态"规划中"标记
        // 实际状态由后端 /api/modules 和 /api/health 动态返回
        const PLANNED_MODULES = new Set();
        ...
        let starredModules = JSON.parse(localStorage.getItem('starredModules')) || [];
    ...
    // ═══════════════════════════════════════════════════════════════
    const EvoAPI = (function() {
        // file://协议下hostname为空，也需要指向后端
        const _port = window.location.port || '8765';
        ...
        async function moduleHealth(name) {
            return _fetch('/api/modules/' + name + '/health');
        }
        ...
        async function auditLog(limit, status) {
            let path = '/api/security/audit?limit=' + (limit || 50);
        ...
        async function buildChain(task) {
            // 模拟构建执行链
            const matched = await findModules(task);
            const chain = matched.slice(0, 4).map(m => ({ module: m.name, action: 'execute', confidence: 0.85 + Math.random() * 0.14 }));
        ...
        async function execute(task) {
            try {
                ...
                const result = await _fetch('/api/execute', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                try {
                    const chainData = await buildChain(task);
                    const chain = (chainData.chain || []).map(c => c.module);
                    if (chain.length > 0) {
        ...
        async function searchModules(query, statusFilter, limit, offset) {
            let path = '/api/search/modules?q=' + encodeURIComponent(query || '') + '&limit=' + (limit || 50) + '&offset=' + (offset || 0);
    ...
    var AutoExecutionEngine = null;
        ...
        function renderNav() {
            ...
            groups['协调引擎'] = [];
            if (groups['系统大脑']) {
                const idx = groups['系统大脑'].findIndex(m => m.id === 'billion-group-os');
            ...
            navGroups.innerHTML = sortedEntries.map(([name, items]) => `
                <div class="nav-group ${collapsedGroups.includes(name) ? 'collapsed' : ''}" data-group="${name}">
                    <div class="nav-group-header" onclick="toggleGroup('${name}')" style="${name === '协调引擎' ? 'color:var(--accent);' : ''}">
                        <span class="nav-group-title">${name === '协调引擎' ? '🧠 ' : ''}${name}</span>
        ...
        function renderDashboard() {
            ...
            // 从后端获取实时数据
            const backendStatus = EvoAPI.statusData;
            const loadedCount = backendStatus ? (backendStatus.modules_loaded || 0) : 0;
            const totalCount = backendStatus ? (backendStatus.modules_total || 0) : 0;
            let uptime = 0;
            if (backendStatus) {
                if (backendStatus.uptime_seconds) uptime = backendStatus.uptime_seconds;
                else if (backendStatus.uptime) { const d = new Date(backendStatus.uptime); uptime = Math.max(0, Math.round((Date.now() - d.getTime()) / 1000)); }
            }
            const uptimeStr = uptime >= 3600 ? Math.floor(uptime/3600) + 'h ' + Math.floor((uptime%3600)/60) + 'm' : Math.floor(uptime/60) + 'm ' + (uptime%60) + 's';
            const isOnline = EvoAPI.isConnected();
            ...
            document.getElementById('content').innerHTML = `
                ...
                ${sortedEntries.map(([name, items]) => `
                    ...
                    <div class="module-grid">
                        ...
                        ${items.map(m => {
                            const isPlanned = PLANNED_MODULES.has(m.id);
                            return `
        ...
        function showPage(id) {
            ...
            const module = [...modules, ...customModules].find(m => m.id === id);
        ...
        function saveModule() {
            ...
            const group = document.getElementById('newModuleGroup').value.trim();

            const icon = document.getElementById('newModuleIcon').value;

            const desc = document.getElementById('newModuleDesc').value.trim();

            const source = document.getElementById('newModuleSource').value.trim();

            const tags = document.getElementById('newModuleTags').value.split(',').map(t => t.trim()).filter(Boolean);
            ...
            const moduleData = { id, name, group, icon, desc, source, tags, color };
            ...
            if (editId) {
                ...
                const customIdx = customModules.findIndex(x => x.id === editId);
                ...
                if (customIdx !== -1) {
                    ...
                    const builtinIdx = modules.findIndex(x => x.id === editId);
        ...
        function confirmAction() {
            ...
            if (pendingAction.type === 'delete') {
                ...
                const wasInModules = modules.some(m => m.id === id);

                const wasInCustom = customModules.some(m => m.id === id);
        ...
        function exportModules() {
            ...
            const data = JSON.stringify(customModules, null, 2);
            ...
            input.onchange = (e) => {
                ...
                reader.onload = (evt) => {
                    ...
                    try {
                        ...
                        const mode = confirm(`导入 ${data.length} 个模块：\n\n「确定」= 合并（追加，不覆盖已有）\n「取消」= 覆盖（清空后导入）`) ? 'merge' : 'replace';
...
(function() {
    ...
    async function loadBackendModuleInfo(moduleId) {
        ...
        if (!area || !EvoAPI.isConnected()) return;
        // 前端用kebab-case，后端用snake_case
        const backendName = moduleId.replace(/-/g, '_');
        try {
            ...
            const name = info?.module_name || info?.name || moduleId;
            const version = info?.version || '-';
            const level = info?.module_level || '-';
            const fileSize = info?.file_size ? (info.file_size / 1024).toFixed(1) + ' KB' : '-';
            const totalLines = codeInfo?.total_lines || '-';
            const docPreview = codeInfo?.docstring_preview || '';
            area.innerHTML = `
                <div class="detail-section">
    ...
    async function doModuleHealth(frontendId) {
        const area = _resultArea();
        const name = _getBackendName(frontendId);
        area.innerHTML = '<div style="padding:12px;color:var(--text-muted);">⏳ 正在检查 ' + name + ' ...</div>';
        try {
            const data = await EvoAPI.moduleHealth(name);
            const healthy = data.healthy || data.status === 'ok' || data.status === 'healthy';
    ...
    async function runModuleExecute(frontendId) {
        const name = _getBackendName(frontendId);
        const action = document.getElementById('exec-action').value.trim();
        const paramsStr = document.getElementById('exec-params').value.trim();
        ...
        try {
            const data = await EvoAPI.executeModule(name, action, params);
    ...
    async function doModuleCode(frontendId) {
        ...
        try {
            const data = await EvoAPI.getModuleCode(name);
            const code = data.source_code || data.code || '(无源码)';
    ...
    async function doModuleActions(frontendId) {
        ...
        try {
            const data = await EvoAPI.getModule(name);
            const methods = [];
    ...
    async function quickExec(frontendId, action) {
        ...
        try {
            const data = await EvoAPI.executeModule(name, action, {});
            const success = data.success !== false;
            area.innerHTML = `
    ...
    window.updateSystemStatus = function(status) {
        let indicator = document.getElementById('evo-system-status');
        if (!indicator) {
    ...
    function updateModuleHealth(healthData) {
        document.querySelectorAll('.module-card').forEach(function(card) {
            ...
            if (!moduleId) return;
            const health = healthData[moduleId.replace(/-/g, '_')];
            if (health) {
                let dot = card.querySelector('.module-status');
                if (!dot) {
    ...
    async function loadModuleStatus() {
        try {
            const data = await EvoAPI.listModules();
            const count = data.count || data.total || (data.modules && data.modules.length) || 0;
    ...
    async function loadV3Status() {
        try {
            ...
            // 更新悬浮状态卡
            let panel = document.getElementById('v3-coord-panel');
            if (!panel) {
                panel = createV3Panel();
            }

            const score = (caps && caps.automation_score) || status.automation_score || 0;
            const modCount = (status.modules && status.modules.registered) || 0;
            const capCount = (caps && caps.capabilities_count) || 0;
            const autoLoop = (status.capabilities && status.capabilities.autonomous_loop) || false;
    ...
    function openMonitorPanel() {
        let modal = document.getElementById('sys-monitor-modal');
    ...
    async function _pollMonitor() {
        try {
            const r = await fetch('/api/monitor/realtime');
            if (!r.ok) return;
            ...
            let epHtml = '<table style="width:100%;border-collapse:collapse;">';
            ...
            for (const e of (ep.top_by_requests || []).slice(0, 8)) {
                const lat = ep.avg_latency_ms[e.path] || '—';
                const latColor = typeof lat === 'number' && lat > 1000 ? '#ef4444' : typeof lat === 'number' && lat > 200 ? '#f59e0b' : '#10b981';
            ...
            // 延迟图表
            const latencies = Object.entries(ep.avg_latency_ms || {}).map(([p, v]) => ({ path: p.split('/').pop() || p, ms: v })).sort((a, b) => b.ms - a.ms).slice(0, 10);
            if (latencies.length) {
                const maxMs = Math.max(...latencies.map(l => l.ms), 1);
                let chartHtml = '';
                for (const l of latencies) {
                    const h = Math.max(4, (l.ms / maxMs) * 100);
                    const c = l.ms > 1000 ? '#ef4444' : l.ms > 200 ? '#f59e0b' : '#10b981';
    ...
    function openV3Panel() {
        ...
        // 创建模态框
        let modal = document.getElementById('v3-coord-modal');
        if (modal) { console.log('[openV3Panel] show existing modal'); modal.style.display = 'flex'; if (_v3Minimized) { const d = document.getElementById('v3-coord-dialog'); if (d) d.classList.remove('v3-minimized'); _v3Minimized = false; } loadV3Modal(); AutoExecutionEngine.fetchBackendStatus(); return; }
        ...
        modal.innerHTML = `
            <div class="resizable-modal v3-dark" id="v3-coord-dialog" style="background:var(--v3-bg);border:1px solid var(--v3-border);border-radius:20px;width:92%;max-width:1100px;height:82vh;display:flex;flex-direction:column;color:var(--v3-text);font-family:system-ui;box-shadow:0 20px 60px rgba(0,0,0,0.4);position:relative;margin:auto;">
                ...
                <div class="v3-bottom-bar" id="v3-bottom-bar">
                    ...
                    <span>564 模块 · <span id="v3-score-display">--</span>/100 · <span id="v3-uptime-display"></span></span>
    ...
    // 记住弹窗位置/大小状态
    let _v3Maximized = false;
    let _v3SavedRect = null;
    let _v3Minimized = false;
    let _v3Dark = true;

    function toggleV3Theme() {
        const dialog = document.getElementById('v3-coord-dialog');
        const btn = document.getElementById('v3-theme-btn');
        if (!dialog) return;
        _v3Dark = !_v3Dark;
    ...
    function initV3DragResize() {
        const dialog = document.getElementById('v3-coord-dialog');
        const header = document.getElementById('v3-coord-header');
        const handle = document.getElementById('v3-resize-handle');
    ...
    async function loadV3Modal() {
        const content = document.getElementById('v3-modal-content');
        if (!content) return;
        try {
            const [status, caps] = await Promise.all([
                EvoAPI.getCoordinatorStatus().catch(() => null),
            ...
            const score = (caps && caps.automation_score) || 0;
            const stats = status.execution_stats || {};
            const caps_list = (caps && caps.capabilities) || {};
            const tags_list = (caps && caps.tags) || {};
            const sd = document.getElementById('v3-score-display'); if(sd) sd.textContent = score;
            const ut = document.getElementById('v3-uptime-display'); if(ut) ut.textContent = new Date().toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'});
    ...
    async function v3FindModules() {
        const input = document.getElementById('v3-find-input');
        const results = document.getElementById('v3-find-results');
        if (!input || !results) return;
        const q = input.value.trim();
        if (!q) return;
        results.innerHTML = '查找中...';
        try {
            const data = await EvoAPI.findModules(q);
            if (!data.matches || data.matches.length === 0) {
                results.innerHTML = '<span style="color:#9ca3af;">未找到匹配的模块</span>';
                return;
            }
            let html = '<div style="display:flex;flex-direction:column;gap:4px;">';
            for (const m of data.matches.slice(0, 8)) {
                const tags = (m.tags || []).map(t => `<span style="background:#3b82f6;border-radius:8px;padding:1px 6px;font-size:10px;">${t}</span>`).join('');
    ...
    async function v3ExecuteTask() {
        const input = document.getElementById('v3-task-input');
        const chainEl = document.getElementById('v3-exec-chain');
        const resultEl = document.getElementById('v3-exec-result');
        if (!input || !chainEl || !resultEl) return;
        const task = input.value.trim();
        if (!task) { resultEl.innerHTML = '<span style="color:#f59e0b;">请输入任务</span>'; return; }

        chainEl.innerHTML = '<span style="color:#9ca3af;">⚡ 正在调度执行链...</span>';
        resultEl.innerHTML = '<span style="color:#9ca3af;">等待结果...</span>';

        try {
            const r = await EvoAPI.execute(task);
            const success = r.success;

            // 从后端返回的step_results构建执行链展示
            const steps = r.step_results || [];
            if (steps.length > 0) {
                const chainHtml = steps.map((s, i) => {
                    const ok = s.result && s.result.success !== false;
                    return `<span style="display:inline-flex;align-items:center;gap:4px;">
                        <span style="background:${ok ? '#3b82f6' : '#6b7280'};border-radius:6px;padding:2px 8px;font-size:11px;color:#fff;">${s.module}</span>
                        <span style="color:${ok ? '#10b981' : '#ef4444'};font-size:10px;">${ok ? '✓' : '✗'}</span>
                        ${i < steps.length - 1 ? '<span style="color:#3b82f6;margin:0 2px;">→</span>' : ''}
                    </span>`;
                }).join('');
                chainEl.innerHTML = `<div style="background:#0f0f1a;border-radius:8px;padding:10px 12px;">
                    <span style="color:#6b7280;font-size:11px;margin-right:8px;">执行链:</span>${chainHtml}
                </div>`;
            } else if (r.chain && r.chain.length > 0) {
                const chainArr = Array.isArray(r.chain) ? r.chain : [];
                chainEl.innerHTML = `<div style="background:#0f0f1a;border-radius:8px;padding:10px 12px;">
                    <span style="color:#6b7280;font-size:11px;margin-right:8px;">执行链:</span>
                    ${chainArr.map(c => `<span style="background:#3b82f6;border-radius:6px;padding:2px 8px;font-size:11px;color:#fff;margin-right:4px;">${c}</span>`).join('<span style="color:#3b82f6;margin:0 2px;">→</span>')}
                </div>`;
            } else {
                chainEl.innerHTML = '<span style="color:#6b7280;font-size:11px;">暂无链路信息</span>';
            }
            ...
            if (payload.repos && payload.repos.length > 0) {
                resultText = payload.repos.map(repo =>
                    `⭐ ${repo.stars || 0}  ${repo.name}\n   ${(repo.description || '').substring(0, 80)}\n   ${repo.url || ''}`
                ).join('\n\n');
            } else if (typeof payload === 'object') {
                const rj = JSON.stringify(payload, null, 2);
                resultText = rj.length > 600 ? rj.slice(0, 600) + '\n...' : rj;
            } else {
                resultText = String(payload);
            }
            if (!resultText && r.error) resultText = r.error;

            // suggestion提示（当任务超出系统领域时）
            const suggestHtml = (!success && r.suggestion)
                ? `<div style="margin-top:8px;padding:8px 12px;background:rgba(59,130,246,0.1);border:1px solid rgba(59,130,246,0.3);border-radius:8px;color:#93c5fd;font-size:12px;">💡 ${r.suggestion}</div>`
                : '';

            resultEl.innerHTML = `<div style="background:#0f172a;border-radius:8px;padding:12px;margin-top:6px;">
                <b style="color:${success ? '#10b981' : '#ef4444'};font-size:13px;">${success ? '✅ 执行成功' : '❌ 执行失败'}</b>
                <pre style="color:#d1d5db;margin-top:8px;overflow:auto;max-height:240px;font-size:11px;line-height:1.6;white-space:pre-wrap;">${resultText || '(无返回内容)'}</pre>
                ${suggestHtml}
            </div>`;
        } catch (e) {
            chainEl.innerHTML = '<span style="color:#ef4444;">调度失败</span>';
            resultEl.innerHTML = `<span style="color:#ef4444;">执行失败: ${e.message}</span>`;
        }
    }

    async function v3TestExecute() {
        const input = document.getElementById('v3-task-input');
        if (input) input.value = '查询上证指数今天的收盘价';
        v3ExecuteTask();
    }
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            const m = document.getElementById('v3-coord-modal');
            if (m) m.style.display = 'none';
        }
    });

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => { initSystemIntegration(); initV3Coordinator(); });
    } else {
        initSystemIntegration();
        initV3Coordinator();
    }
    // 暴露IIFE内函数到全局（供HTML onclick调用）
    window.openV3Panel = openV3Panel;
    window.loadV3Modal = loadV3Modal;
    window.toggleV3Maximize = toggleV3Maximize;
    window.toggleV3Theme = toggleV3Theme;
    window.toggleV3Minimize = toggleV3Minimize;

    // ── 模块管线引擎 (Pipeline Studio) ──
    let _pipelineList = [];
    let _pipelineExecutions = [];
    ...
    async function openPipelineStudio() {
        ...
        overlay.innerHTML = `
        <div id="pipeline-dialog" style="background:var(--card);border-radius:16px;width:90%;max-width:1100px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;border:1px solid var(--border);">
            <div style="padding:16px 20px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:12px;flex-shrink:0;">
                ...
                <button onclick="pipelineShowExecHistory()" style="padding:6px 14px;background:var(--bg);color:var(--text);border:1px solid var(--border);border-radius:8px;cursor:pointer;font-size:12px;">📋 历史</button>
                <button onclick="pipelineNewForm()" style="padding:6px 14px;background:var(--primary);color:#fff;border:none;border-radius:8px;cursor:pointer;font-size:12px;">+ 新建</button>
            ...
            <div id="pipeline-body" style="flex:1;overflow-y:auto;padding:20px;"></div>
        </div>`;
    ...
    async function pipelineRefresh() {
        try {
            ...
            const d = await r.json();
            _pipelineList = d.pipelines || [];
        } catch(e) { _pipelineList = []; }
        pipelineRenderList();
    }

    function pipelineRenderList() {
  ...
  {"id":"s3","name":"发送报告","type":"notify","channel":"email","message_template":"分析完成: {{analysis_result}}"}
    ...
    async function pipelineCreateSubmit() {
        ...
        const stepsStr = document.getElementById('pl-steps').value.trim();
        ...
        const tags = document.getElementById('pl-tags').value.split(',').map(s=>s.trim()).filter(Boolean);
        try {
            const r = await fetch('/api/pipelines', {
    ...
    async function pipelineExecute(plId) {
        ...
        try {
            const r = await fetch('/api/pipelines/' + plId + '/execute', {
    ...
    function pipelineShowExecResult(d) {
        const body = document.getElementById('pipeline-body');
        if (!body) return;
        const statusColor = d.status === 'success' ? 'var(--success)' : d.status === 'failed' ? 'var(--danger)' : 'var(--warning)';
        const steps = d.steps || {};
        body.innerHTML = `
        <div style="max-width:700px;margin:0 auto;">
            <button onclick="pipelineRefresh()" style="margin-bottom:16px;padding:6px 14px;background:var(--bg);color:var(--text);border:1px solid var(--border);border-radius:8px;cursor:pointer;font-size:12px;">← 返回列表</button>
            ...
            <div style="display:flex;flex-direction:column;gap:8px;">
                ${Object.entries(steps).map(([sid, sr]) => {
                    const sc = sr.status==='success'?'var(--success)':sr.status==='failed'?'var(--danger':'var(--text-muted)';
    ...
    async function pipelineShowExecHistory(plId) {
        const url = plId ? '/api/pipelines/executions?pipeline_id=' + plId + '&limit=20' : '/api/pipelines/executions?limit=20';
        try {
            ...
            <div style="max-width:700px;margin:0 auto;">
                ...
                `<div style="display:flex;flex-direction:column;gap:6px;">
                    ${execs.map(ex => {
                        const sc = ex.status==='success'?'var(--success)':ex.status==='failed'?'var(--danger':'var(--warning)');
    ...
    async function pipelineViewExecDetail(execId) {
        try {
            const r = await fetch('/api/pipelines/executions/' + execId);
            const d = await r.json();
            if (d.success) {
                // 复用执行结果展示
                const steps = {};
                try {
                    const parsed = typeof d.steps_result_json === 'string' ? JSON.parse(d.steps_result_json) : d.steps_result_json;
    ...
    function configRender() {
        ...
        const groupLabels = {llm:'🤖 LLM模型', notify:'📢 通知渠道', cicd:'🚀 CI/CD', database:'🗄 数据库', custom:'📦 自定义'};
        const groupColors = {llm:'#6366f1', notify:'#f59e0b', cicd:'#10b981', database:'#06b6d4', custom:'#8b5cf6'};
    ...
    async function configSetItem(key) {
        ...
        if (!input) return;
        try {
            const r = await fetch('/api/config/' + key, {method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify({value:input.value})});
            const d = await r.json();
    ...
    async function configSaveAll() { const r = await fetch('/api/config/save',{method:'POST'}); showToast('所有配置已持久化到文件'); }
    ...
    async function schedulerCreateSubmit() {
        const body = {name:document.getElementById('sch-name').value, target_type:document.getElementById('sch-target-type').value, target_id:document.getElementById('sch-target-id').value};
        const cron = document.getElementById('sch-cron').value.trim();
        if (cron) body.cron = cron;
        else body.interval = 300;
        try {
            const r = await fetch('/api/scheduler/tasks',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
            const d = await r.json();
    ...
    function eventNewRule() { showToast('规则创建: 请通过API /api/events/rules POST创建'); }
    ...
    async function queueRefresh() {
        try {
            ...
            const statusColors = {pending:'var(--text-muted)',running:'var(--primary)',success:'var(--success)',failed:'var(--danger)',retrying:'var(--warning)',timeout:'var(--warning)',cancelled:'var(--text-muted)',dead:'#ef4444'};
            const statusLabels = {pending:'等待中',running:'执行中',success:'成功',failed:'失败',retrying:'重试中',timeout:'超时',cancelled:'已取消',dead:'死信'};
            body.innerHTML = `
    ...
    function queueNewTask() { showToast('任务提交: 请通过API /api/queue/tasks POST创建'); }
    ...
    function wsConnect() {
        ...
        _wsConn = new WebSocket(`${protocol}//${location.host}/ws?rooms=system,events,tasks,logs,alerts`);
        ...
        _wsConn.onmessage = (e) => {
            try {
                ...
                const colors = {log:'#94a3b8',event:'#6366f1',progress:'#f59e0b',status:'#10b981',alert:'#ef4444',notification:'#3b82f6'};
                const color = colors[msg.type] || 'var(--text)';
    ...
    async function startTunnel(backend) {
        ...
        let token = '';
        if (backend === 'pyngrok') {
            const inp = document.getElementById('ngrok-token-input');
        ...
        try {
            const r = await fetch('/api/tunnel/start?backend='+backend+'&auth_token='+encodeURIComponent(token), {method:'POST'});
            const d = await r.json();
            if (d.success) {
                _tunnelActive = true;
    ...
    async function checkTunnelStatus() {
        try {
            ...
            const d = await r.json();
            if (d.active) { _tunnelActive = true; const btn = document.getElementById('tunnel-btn'); if(btn) btn.style.color='#10b981'; }
        } catch(e) {}
    }
...
(function() {
    ...
    const AGENT_ROLES = ['CEO','CTO','CFO','COO','产品经理','架构师','程序员','设计师','运营专员','销售','客服','财务','HR','数据分析师','内容创作者','投研员'];
    ...
    function bgosLoad() {
        ...
        const raw = localStorage.getItem(BGOS_KEY);
        if (!raw) return;
        try {
            const d = JSON.parse(raw);
            Object.assign(BGOS, d);
            // 确保 nextId 结构完整（兼容旧数据）
            const required = { group: 1, company: 1, dept: 1, user: 2, agent: 1, task: 1, approval: 1 };
            BGOS.nextId = BGOS.nextId || {};
            Object.keys(required).forEach(k => { if (typeof BGOS.nextId[k] !== 'number') BGOS.nextId[k] = required[k]; });
        } catch(e) {}
    }
    function genId(type) { const num = BGOS.nextId[type]++; return type + '-' + String(num).padStart(4,'0'); }
    ...
    window.openBillionGroupOS = function() {
        ...
        if (BGOS.companies.length === 0) {
            try {
                ...
                const groupName = 'AUTO-EVO-AI 核心集团';
                const companyCount = 3, deptPerCompany = template.depts.length, agentCount = 100, humanCount = 20;
                const group = { id: genId('group'), name: groupName, companies: [], budget: 10000000, valuation: 100000000, status: 'normal' };
                BGOS.groups.push(group);
                for (let i = 0; i < companyCount; i++) {
                    const company = { id: genId('company'), groupId: group.id, name: groupName + ' - 公司' + (i+1), businessType: industry, departments: [], users: [], agents: [], budget: 1000000, income: 0, profit: 0, status: 'running' };
                    BGOS.companies.push(company); group.companies.push(company.id);
                    for (let d = 0; d < deptPerCompany; d++) {
                        const deptName = template.depts[d % template.depts.length];
                        const dept = { id: genId('dept'), companyId: company.id, name: deptName, type: deptName.replace('部',''), manager: null, users: [], agents: [], budget: 100000, kpi: '' };
                        BGOS.departments.push(dept); company.departments.push(dept.id);
                        const deptAgentCount = Math.floor(agentCount / companyCount / deptPerCompany);
                        for (let a = 0; a < deptAgentCount; a++) {
                            const role = AGENT_ROLES[Math.floor(Math.random() * AGENT_ROLES.length)];
                            const agent = { id: genId('agent'), name: role + '-' + (a+1), role, companyId: company.id, deptId: dept.id, skills: [role], status: 'running', budget: 1000, kpi: '完成日常工作', efficiency: 80 + Math.floor(Math.random()*20) };
                            BGOS.agents.push(agent); dept.agents.push(agent.id); company.agents.push(agent.id);
                        }
                    }
                    for (let h = 0; h < Math.floor(humanCount / companyCount); h++) {
                        const roles = ['employee', 'manager', 'admin'];
                        const u = { id: genId('user'), name: '员工' + (h+1), account: 'emp' + (h+1), role: roles[Math.floor(Math.random()*roles.length)], companyId: company.id, deptId: null, status: 'active', type: 'human' };
    ...
    try {
        ...
        const groupName = await showPrompt('集团名称', template.name, '请输入集团名称');
        if (groupName === null) { showToast('输入被取消', 'warning'); return; }
        if (!groupName) return;
        
        const companyCount = parseInt(await showPrompt('创建公司数量', '3', '请输入要创建的公司数量')) || 3;
        const deptPerCompany = parseInt(await showPrompt('每公司部门数量', String(template.depts.length), '请输入每家公司的部门数')) || template.depts.length;
        const agentCount = parseInt(await showPrompt('创建AI智能体数量', '100', '请输入要创建的AI智能体总数')) || 100;
        const humanCount = parseInt(await showPrompt('创建人类员工数量', '20', '请输入要创建的人类员工总数')) || 20;
        ...
        for (let i = 0; i < companyCount; i++) {
            ...
            deptsToCreate.forEach(deptName => {
                ...
                for (let a = 0; a < deptAgentCount; a++) {
                    const role = AGENT_ROLES[Math.floor(Math.random() * AGENT_ROLES.length)];
                    const agent = { id: genId('agent'), name: role + '-' + (a+1), role, companyId: company.id, deptId: dept.id, skills: [role], status: BGOS.autopilot ? 'running' : 'sleeping', budget: 1000, kpi: '完成日常工作', efficiency: 80 + Math.floor(Math.random()*20) };
                    BGOS.agents.push(agent); dept.agents.push(agent.id); company.agents.push(agent.id);
                }
            });
        ...
        bgosUpdateUI();
        const autopilotOn = BGOS.autopilot;
    ...
    try {
        ...
        const deptPerCompany = template.depts.length;
    ...
    window.bgosToggleAutopilot = function() {
        ...
        const btn = document.getElementById('bgos-ap-btn');
    ...
    function bgosAutoLoop() {
        ...
        if (Math.random() > 0.9 && BGOS.agents.length > 0) {
            const agent = BGOS.agents[Math.floor(Math.random() * BGOS.agents.length)];
    ...
    window.bgosUpdateUI = function() {
        ...
        // 空数据引导显示控制
        const guide = document.getElementById('bgos-empty-guide');
        ...
        const val = BGOS.groups.reduce((s,g)=>s+(g.valuation||0),0);
        ...
        // 公司列表
        const cList = document.getElementById('bgos-company-list');
        const cCount = document.getElementById('bgos-c-count');
        ...
        if (cList) {
            if (BGOS.companies.length === 0) { cList.innerHTML = '<p style="text-align:center;color:var(--text-muted);padding:20px;">暂无公司</p>'; }
                ...
                cList.innerHTML = BGOS.companies.map(c => {
                    const group = BGOS.groups.find(g=>g.id===c.groupId);
        ...
        // 部门列表
        const dList = document.getElementById('bgos-dept-list');
        const dCount = document.getElementById('bgos-d-count');
        ...
        if (dList) {
            if (BGOS.departments.length === 0) { dList.innerHTML = '<p style="text-align:center;color:var(--text-muted);padding:20px;">暂无部门</p>'; }
                ...
                dList.innerHTML = BGOS.departments.map(d => {
                    const company = BGOS.companies.find(c=>c.id===d.companyId);
        ...
        // 智能体列表
        const list = document.getElementById('bgos-agent-list');
        if (list) {
            if (BGOS.agents.length === 0) { list.innerHTML = '<p style="text-align:center;color:var(--text-muted);padding:40px;">暂无智能体，点击"一键创世"开始创建</p>'; }
                ...
                list.innerHTML = BGOS.agents.slice(0,50).map(a => {
                    const company = BGOS.companies.find(c=>c.id===a.companyId);
                    const dept = BGOS.departments.find(d=>d.id===a.deptId);
        ...
        const audit = document.getElementById('bgos-audit-log');
        if (audit) {
    ...
    window.bgosShowAgents = function() { bgosUpdateUI(); const el = document.getElementById('bgos-agent-list'); if (el) el.scrollIntoView({behavior:'smooth'}); };

    // ── 删除功能 ──────────────────────────────────────────────
    window.bgosDeleteAgent = function(id) {
        try {
            const agent = BGOS.agents.find(a => a.id === id);
            if (!agent) return;
            if (!confirm('确认删除智能体: ' + agent.name + '?')) return;
            BGOS.agents = BGOS.agents.filter(a => a.id !== id);
            // 从公司和部门引用中移除
            const company = BGOS.companies.find(c => c.id === agent.companyId);
            if (company) company.agents = company.agents.filter(aid => aid !== id);
            const dept = BGOS.departments.find(d => d.id === agent.deptId);
    ...
    window.bgosDeleteDept = function(id) {
        try {
            const dept = BGOS.departments.find(d => d.id === id);
            if (!dept) return;
            ...
            const company = BGOS.companies.find(c => c.id === dept.companyId);
    ...
    window.bgosDeleteCompany = function(id) {
        try {
            const company = BGOS.companies.find(c => c.id === id);
            if (!company) return;
            if (!confirm('确认删除公司: ' + company.name + '?\n(该公司下所有部门、智能体、员工将被删除)')) return;
            // 删除关联的部门
            const deptIds = BGOS.departments.filter(d => d.companyId === id).map(d => d.id);
            ...
            // 从集团引用中移除
            const group = BGOS.groups.find(g => g.id === company.groupId);
    ...
    try {
        ...
        const companyId = await showPrompt('所属公司ID', BGOS.companies.length ? BGOS.companies[0].id : '', '请输入公司ID');
        if (companyId === null) return;
        const name = await showPrompt('部门名称', '技术部', '请输入部门名称');
        if (name === null) return;
        if (!name) { showToast('部门名称不能为空', 'warning'); return; }
        const type = await showPrompt('部门类型', '技术', '请输入部门类型');
        const budget = parseInt(await showPrompt('部门预算', '100000', '请输入部门预算（元）')) || 100000;
        const kpi = await showPrompt('部门KPI', '完成年度目标', '请输入部门KPI');
        const dept = { id: genId('dept'), companyId, name, type: type || '通用', manager: null, users: [], agents: [], budget, kpi: kpi || '' };
        BGOS.departments.push(dept);
        const company = BGOS.companies.find(c => c.id === companyId);
    ...
    try {
        ...
        const companyId = await showPrompt('所属公司ID', BGOS.companies.length ? BGOS.companies[0].id : '', '请输入公司ID');
        if (companyId === null) return;
        const name = await showPrompt('用户姓名', '新员工', '请输入用户姓名');
        if (name === null) return;
        if (!name) { showToast('用户姓名不能为空', 'warning'); return; }
        const account = await showPrompt('账号', 'emp' + (BGOS.users.length + 1), '请输入登录账号');
        const role = await showPrompt('角色', 'employee', '请输入角色（employee/manager/admin）') || 'employee';
        const deptId = await showPrompt('所属部门ID（可选）', '', '请输入部门ID（留空则表示无部门）');
        const u = { id: genId('user'), name, account: account || ('emp' + (BGOS.users.length + 1)), role, companyId, deptId: deptId || null, status: 'active', type: 'human' };
        BGOS.users.push(u);
        const company = BGOS.companies.find(c => c.id === companyId);
    ...
    try {
        ...
        const companyId = await showPrompt('所属公司ID', BGOS.companies.length ? BGOS.companies[0].id : '', '请输入公司ID');
        if (companyId === null) return;
        const name = await showPrompt('智能体名称', '新智能体', '请输入智能体名称');
        if (name === null) return;
        if (!name) { showToast('智能体名称不能为空', 'warning'); return; }
        const role = await showPrompt('角色', 'assistant', '请输入智能体角色（assistant/expert/analyst等）') || 'assistant';
        const deptId = await showPrompt('所属部门ID（可选）', '', '请输入部门ID（留空则表示无部门）');
        const budget = parseInt(await showPrompt('预算', '1000', '请输入预算（元）')) || 1000;
        const kpi = await showPrompt('KPI', '完成日常工作', '请输入KPI目标');
        const agent = { id: genId('agent'), name, role, companyId, deptId: deptId || null, skills: [role], status: BGOS.autopilot ? 'running' : 'sleeping', budget, kpi: kpi || '完成日常工作', efficiency: 80 + Math.floor(Math.random()*20) };
        BGOS.agents.push(agent);
        const company = BGOS.companies.find(c => c.id === companyId);
        if (company) company.agents.push(agent.id);
        if (deptId) {
            const dept = BGOS.departments.find(d => d.id === deptId);
    ...
    window.bgosBatchImportForm = async function() {
        ...
        // 使用内联模态框选择导入类型
        const type = await showPrompt('批量导入', '', '导入类型：1=公司 2=部门 3=用户 4=智能体');
        if (type === null || type === undefined) { await showAlert('导入已取消', '🚫'); return; }
        if (!type) return;
        const typeMap = { '1': 'companies', '2': 'departments', '3': 'users', '4': 'agents' };
        const typeKey = typeMap[type.trim()];
        if (!typeKey) { await showAlert('无效选项，请输入 1-4', '⚠️'); return; }

        // 显示示例数据帮助用户输入
        const sample = type.trim() === '1' ? '[{"name":"新公司","businessType":"saas"}]' :
        ...
        const raw = await showPrompt('粘贴数据', sample, '粘贴 JSON 数组或 CSV 数据', true);
        ...
        try {
            if (raw.trim().startsWith('[')) { items = JSON.parse(raw); }
                ...
                for (let i = 1; i < lines.length; i++) {
                    ...
                    const obj = {}; headers.forEach((h,idx) => obj[h] = vals[idx] ? vals[idx].trim() : '');
        ...
        items.forEach(item => {
            try {
                if (t === '1') {
                    const c = { id: genId('company'), groupId: (BGOS.groups[0] && BGOS.groups[0].id) || null, name: item.name || '未命名公司', businessType: item.businessType || 'saas', departments: [], users: [], agents: [], budget: 1000000, income: 0, profit: 0, status: 'running' };
                    BGOS.companies.push(c); if (BGOS.groups[0]) BGOS.groups[0].companies.push(c.id);
                } else if (t === '2') {
                    const company = BGOS.companies.find(c => c.id === item.companyId);
                    if (!company) return;
                    const d = { id: genId('dept'), companyId: company.id, name: item.name || '未命名部门', type: item.type || (item.name||'').replace('部',''), manager: null, users: [], agents: [], budget: 100000, kpi: '' };
                    BGOS.departments.push(d); company.departments.push(d.id);
                } else if (t === '3') {
                    const company = BGOS.companies.find(c => c.id === item.companyId);
                    if (!company) return;
                    const u = { id: genId('user'), name: item.name || '未命名', account: item.account || item.name || 'user', role: item.role || 'employee', companyId: company.id, deptId: item.deptId || null, status: 'active', type: 'human' };
                    BGOS.users.push(u); company.users.push(u.id);
                } else if (t === '4') {
                    const company = BGOS.companies.find(c => c.id === item.companyId);
                    if (!company) return;
                    const skills = Array.isArray(item.skills) ? item.skills : (item.skills || '通用').split(/[,，]/).map(s => s.trim()).filter(Boolean);
                    const a = { id: genId('agent'), name: item.name || '未命名AI', role: item.role || '通用助手', companyId: company.id, deptId: item.deptId || null, skills: skills, status: 'sleeping', budget: 1000, kpi: '完成日常工作', efficiency: 80 + Math.floor(Math.random()*20) };
...
(function() {
    ...
    const EventBus = {
        ...
        off(evt, fn) { if (!this._events[evt]) return; const i = this._events[evt].indexOf(fn); if (i > -1) this._events[evt].splice(i, 1); },
    ...
    const AgentOrchestrator = {
        ...
        // 为每个运行中的智能体分配下一个任务
        assignTask(agent) {
            const pipeline = this.rolePipeline[agent.role] || ['task-center'];
            // 根据智能体状态和业务需求选择模块
            const moduleId = pipeline[Math.floor(Math.random() * pipeline.length)];
            const mod = ModuleRegistry[moduleId];
            if (!mod) return null;

            // 模拟任务执行
            const taskType = mod.events[Math.floor(Math.random() * mod.events.length)];
            return {
                agentId: agent.id,
        ...
        orchestrate() {
            ...
            const agents = (window.BGOS && window.BGOS.agents) ? window.BGOS.agents.filter(a => a.status === 'running') : [];
    ...
    const AutoExecutionEngine = {
        ...
        async fetchBackendStatus() {
            try {
                if (typeof EvoAPI !== 'undefined') {
                    const status = await EvoAPI.health();
                    if (status) {
                    ...
                    try {
                        const coord = await EvoAPI.getCoordinatorStatus();
                        if (coord && coord.execution_stats) {
                            // 后端有真实数据时用后端数据，否则保留前端累计值
                            const bt = coord.execution_stats.total || coord.execution_stats.tasks_executed || 0;
                            const bm = coord.execution_stats.success || coord.execution_stats.modules_triggered || 0;
        ...
        async pollBackend() {
            ...
            try {
                ...
                const status = await EvoAPI.getCoordinatorStatus();
                if (!status) throw new Error('协调器未响应');
                this.backendOnline = true;
                this.backendError = null;
                if (status && status.execution_stats) {
                    const bt = status.execution_stats.total || status.execution_stats.tasks_executed || 0;
                    const bm = status.execution_stats.success || status.execution_stats.modules_triggered || 0;
                ...
                const recents = status && status.recent_executions ? status.recent_executions : [];
                if (recents.length > 0) {
                    recents.forEach(ex => {
                        const mod = ex.module || ex.method || '未知';
                        const success = ex.success ? '成功' : '失败';
                ...
                const picks = Math.floor(Math.random() * 4) + 2;
                for (let i = 0; i < picks; i++) {
                    const mod = SIM_MODULES[Math.floor(Math.random() * SIM_MODULES.length)];
                    const actions = ['执行完成', '处理成功', '分析结束', '调度完毕', '同步完成'];
        ...
        logEvent(category, message) {
            const entry = { time: new Date().toLocaleTimeString('zh-CN'), category, message, tick: this.tickCount };
            this.eventLog.unshift(entry);
            if (this.eventLog.length > 200) this.eventLog.pop();
        },

        updateCoordinationUI() {
            const panel = document.getElementById('coord-metrics');
            if (panel) {
                const backendStatus = !this.running ? (this.backendOnline ? '🟢 待命' : '⚪ 离线') : this.backendOnline ? '🟢 运行中' : (this.backendError || '连接中...');
                const backendColor = this.backendOnline ? '#10b981' : this.running ? '#ef4444' : 'var(--text-muted)';
                const loaded = this._loadedModules || 0;
                const total = this._totalModules || 0;
                panel.innerHTML = `
                    <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:12px;">
            ...
            const logEl = document.getElementById('coord-event-log');
            if (logEl) {
            ...
            const modEl = document.getElementById('coord-module-activity');
            if (modEl) {
                const activity = {};
                this.eventLog.forEach(e => {
                    if (e.category === 'TASK') {
                        const match = e.message.match(/\[后端\]\s*([^\s]+)/);
    ...
    // ═══════════════════════════════════════════════════════════════
    const API_BASE = `http://127.0.0.1:${window.location.port || '8765'}`;
    const BackendClient = {
        ws: null,
        connected: false,
        reconnectTimer: null,
        ...
        async init() {
            // 测试后端连通性
            try {
                const r = await fetch(API_BASE + '/api/health', { mode: 'cors', cache: 'no-store' });
                if (r.ok) {
                    const d = await r.json();
        ...
        connectWS() {
            ...
            try {
                this.ws = new WebSocket(`ws://127.0.0.1:${window.location.port || '8765'}/ws`);
                this.ws.onopen = () => {
                    console.log('[WS] 已连接');
                    this.connected = true;
                    // 请求协调器状态
    ...
    const _origTick = AutoExecutionEngine.tick.bind(AutoExecutionEngine);
    AutoExecutionEngine.tick = async function() {
        _origTick();
        // 每10个tick拉一次后端状态
        if (this.tickCount % 10 === 0 && this.backendOnline) {
            const status = await BackendClient.get('/api/coordinator/status');
            if (status) {
    ...
    AutoExecutionEngine.updateCoordinationUI = function() {
        const bc = this.backendCoordinator || {};
        const healthOk = Object.values(this.backendHealth).filter(h => h && h.status === 'ok').length;
        const healthTotal = Object.keys(this.backendHealth).length;
        ...
        if (modEl) {
            ...
            this.eventLog.forEach(e => {
                if (e.category === 'TASK' || e.category === 'BACKEND') {
                    ...
                    const m1 = e.message.match(/→\s*([^[]+)\[/);
                    if (m1) { const n = m1[1].trim(); activity[n] = (activity[n]||0)+1; return; }
                    // 格式2: pollBackend [后端] module -> 成功
                    const m2 = e.message.match(/\[后端\]\s*([^\s]+)/);
                    if (m2) { activity[m2[1]] = (activity[m2[1]]||0)+1; return; }
                    // 格式3: [后端执行] {模块名...}
                    const m3 = e.message.match(/\[后端执行\].*?(\w+_\w+|\w+)/);
    ...
    // ── 监听BGOS全自动开关，联动协调引擎 ─────────────────────────
    const _origToggle = window.bgosToggleAutopilot;
    window.bgosToggleAutopilot = function() {
    ...
    window.openCoordinationPanel = function() {
        try {
            ...
            const regCount = Object.keys(ModuleRegistry || {}).length;
            const modulesHtml = Object.entries(ModuleRegistry || {}).map(([id, mod]) => {
                const ev = (mod.events || []).length;
                const ou = (mod.outputs || []).length;
    ...
    window.showPage = function(page, el) {
        if (page === 'coordination') {
            try {
                ...
                const fb = document.getElementById('floatBackBtn');
...
(function() {
    ...
    window._exSwitchTab = function(tab) {
        ...
        const el = document.getElementById('ex-tab-' + tab);
        if (el) el.classList.add('active');
    ...
    async function loadExplorerModules() {
        ...
        if (!container) return;
        const s = _explorerState;
        const query = (container.querySelector('#ex-search') || {}).value || s.query;
        const statusFilter = (container.querySelector('#ex-status') || {}).value || s.statusFilter;
        const categoryFilter = (container.querySelector('#ex-category') || {}).value || s.categoryFilter;
        s.query = query;
        ...
        try {
            ...
            const data = await EvoAPI.searchModules(query, statusFilter, 5000, 0);
            ...
            const cats = {};
            mods.forEach(m => {
                const c = m.category || classifyLocal(m.name);
                cats[c] = (cats[c] || 0) + 1;
            });

            const statsHtml = Object.entries(cats).sort((a,b) => b[1]-a[1]).slice(0, 8).map(([c,n]) =>
                `<span class="badge" style="background:var(--bg-tertiary);color:var(--text-muted);font-size:11px;">${escHtml(c)}: ${n}</span>`
            ).join('');
            ...
            if (mods.length > 100) {
                ...
                wrap.addEventListener('scroll', function() {
                    if (wrap.scrollTop + wrap.clientHeight > wrap.scrollHeight - 200 && loaded < mods.length) {
                        const next = mods.slice(loaded, loaded + 100);
    ...
    function renderModuleTable(mods, total) {
        const badge = document.querySelector('#ex-stats .badge');
        ...
        let html = '<table class="explorer-table"><thead><tr>' +
            ...
            '<th>模块名</th><th>分类</th><th>类名</th><th>状态</th><th>评分</th><th>方法数</th><th>操作</th></tr></thead><tbody id="ex-tbody">';
    ...
    function buildModuleRows(mods) {
        return mods.map(m => {
            const dotColor = m.status === 'ok' || m.status === 'configured' || m.status === 'healthy' ? '#10b981' :
                             m.status === 'pending_lazy' ? '#f59e0b' :
                             m.status === 'error' || m.status === 'timeout' || m.status === 'lazy_error' ? '#ef4444' : '#6b7280';
            const gradeColor = m.grade === 'A' || m.grade === 'S' ? '#10b981' : m.grade === 'B' ? '#f59e0b' : m.grade === 'C' ? '#f97316' : '#ef4444';
            const cat = m.category || classifyLocal(m.name);
            return `<tr>
                ...
                <td><span class="badge" style="font-size:10px;background:var(--bg-tertiary);color:var(--text-muted);">${escHtml(cat)}</span></td>
    ...
    window._exRescan = async function() {
        try {
            const r = await fetch('/api/modules/rescan', {method: 'POST'});
            const d = await r.json();
            if (d.success) {
    ...
    window._exExecOne = async function(name) {
        try {
            const r = await EvoAPI.executeModule(name, 'status');
            if (r && r.success) {
    ...
    window._exDetail = async function(name) {
        try {
            const r = await EvoAPI.getModule(name);
            if (!r) { showToast('无法加载模块信息', 'error'); return; }
            const d = r.module || r;
            const methods = (d.methods || []).slice(0, 20).map(m => '<li style="padding:2px 0;">' + escHtml(m) + '</li>').join('');
            showModal(name + ' 详情', `
    ...
    function showBatchPanel() {
        const container = document.getElementById('ex-content');
        if (!container) return;
        const sel = _explorerState.selected;
        container.innerHTML = `
    ...
    window._exBatchRun = async function() {
        const action = (document.getElementById('ex-batch-action') || {}).value || 'status';
        const targets = Array.from(_explorerState.selected).map(name => ({ module: name, action: action, params: {} }));
        if (targets.length === 0) { showToast('没有选择模块', 'error'); return; }

        const resultsDiv = document.getElementById('ex-batch-results');
        resultsDiv.innerHTML = '<div class="explorer-empty">&#9203; 执行中...</div>';

        try {
            const data = await EvoAPI.batchExecute(targets);
            const results = data.results || [];
            let html = '<div style="margin-bottom:12px;display:flex;gap:12px;">';
    ...
    window._exRefreshLog = async function() {
        const el = document.getElementById('ex-log-content');
        if (!el) return;
        try {
            const data = await EvoAPI.getExecutionLog(100);
            const logs = data.log || [];
            if (logs.length === 0) {
                el.innerHTML = '<div class="explorer-empty">&#128203; 暂无执行记录</div>';
                return;
            }
            let html = '<table class="explorer-table explorer-log"><thead><tr><th>时间</th><th>模块</th><th>Action</th><th>状态</th><th>耗时</th><th>结果</th></tr></thead><tbody>';
            logs.forEach(l => {
                const cls = l.status === 'ok' ? 'log-ok' : l.status === 'error' ? 'log-error' : 'log-fail';
...
/* ─── SETUP WIZARD LOGIC ─── */
const SW_STEPS = ['welcome', 'ai', 'notify', 'schedule', 'complete'];
let swStep = 0;
let swData = {};
let swChecked = {};
...
function checkSetupWizard() {
  fetch('/api/config/items?group=llm').then(r => r.json()).then(data => {
    ...
    const hasKey = items.some(i => i.key && i.value && i.value.length > 10 && !i.value.startsWith('sk-') === false || (i.value && i.value.length > 10));
    // Show wizard on first visit (no llm config saved)
    const wizardDone = localStorage.getItem('evo_setup_wizard_done');
...
function swRender() {
  ...
  const renderers = { welcome: swRenderWelcome, ai: swRenderAI, notify: swRenderNotify, schedule: swRenderSchedule, complete: swRenderComplete };
...
function swRenderWelcome(el) {
  el.innerHTML = `
    ...
    <div class="sw-welcome-desc">
      智能自动化平台已就绪，564个功能模块 + 12个核心引擎<br>
      只需几步配置即可激活全部能力
    </div>
...
function swValidateAI() {
  const d = document.getElementById('sw_deepseek_key').value.trim();
  const z = document.getElementById('sw_zhipu_key').value.trim();
  const o = document.getElementById('sw_openai_key').value.trim();
  const a = document.getElementById('sw_anthropic_key').value.trim();
  const g = document.getElementById('sw_gemini_key').value.trim();
  swData.llm = {};
...
function swRenderComplete(el) {
  const aiCount = Object.keys(swData.llm || {}).filter(k => !k.includes('url')).length;
  const notifyKeys = ['wecom_webhook','dingtalk_webhook','feishu_webhook','serverchan_key','bark_url','pushplus_token','custom_webhook'];
  const notifyCount = notifyKeys.filter(k => swData.notify[k] && swData.notify[k].length > 5).length;
  const schedChecked = (['sched_health','sched_security','sched_perf','sched_log','sched_filewatch','sched_alert']).filter(id => {
...
async function swNext() {
  ...
  if (step === 'ai') {
    ...
    const a = document.getElementById('sw_anthropic_key').value.trim();
    const g = document.getElementById('sw_gemini_key').value.trim();
    const ou = document.getElementById('sw_ollama_url').value.trim();
    ...
    const filled = Object.values(swData.notify).filter(v => v && v.length > 3);
    if (filled.length > 0) {
...
const WSRealtime = {
    ...
    updateStatus(state) {
        if (!this.statusEl) {
            ...
            const header = document.querySelector('.header-title') || document.querySelector('h1') || document.body;
    ...
    showToast(title, body, type) {
        ...
        const colors = { success: '#22c55e', error: '#ef4444', warning: '#eab308', info: '#6366f1' };
