    ...
    <el-row :gutter="12" style="margin-bottom:16px">
      <el-col :span="4" v-for="s in ['total','pending','running','completed','failed','backlog']" :key="s">
        <el-card shadow="never" class="mini-stat" :body-style="{padding:'12px'}">
          <div class="ms-val" :style="{color:colors[s]||'#e2e8f0'}">{{ stats[s]||0 }}</div>
          <div class="ms-lbl">{{ labels[s]||s }}</div>
        </el-card>
      </el-col>
    </el-row>
    <el-card shadow="never" class="page-card">
      <template #header><div class="card-header"><span>📋 队列任务</span><el-button size="small" type="primary" @click="showEnqueue=true">入队</el-button></div></template>
...
const load = async () => { try { const [qs, qt] = await Promise.all([getQueueStats(), getQueueTasks()]); stats.value = qs; tasks.value = qt.tasks||[] } catch {} }
