...
import logging
logger = logging.getLogger("evo._vosk_worker")
...
def main():
    if len(sys.argv) < 2:
        logger.info(json.dumps({"success": False, "error": "no_wav_path"}))
        return
    wav_path = sys.argv[1]
    if not os.path.isfile(wav_path):
        logger.info(json.dumps({"success": False, "error": "wav_not_found"}))
        return
    try:
        ...
        logger.info(json.dumps({"success": False, "error": f"read_fail:{e}"}))
        return

    # Parse WAV
    if raw[:4] != b"RIFF" or raw[8:12] != b"WAVE":
        logger.info(json.dumps({"success": False, "error": "not_wav"}))
        return
    ...
    try:
        ...
        logger.info(json.dumps({"success": False, "error": f"wav_parse:{e}"}))
        return

    if not pcm or len(pcm) < 2000:
        logger.info(json.dumps({"success": False, "error": "pcm_too_short"}))
        return
    ...
    try:
        ...
        if full:
            logger.info(json.dumps({"success": True, "text": full}))
            return
        ...
        if rec2.AcceptWaveform(pcm):
            ...
            t = r.get("text", "").strip()
            if t:
                logger.info(json.dumps({"success": True, "text": t}))
                return
        ...
        logger.info(json.dumps({"success": False, "error": "no_text"}))
    except Exception as e:
        logger.info(json.dumps({"success": False, "error": f"vosk_err:{e}"}))
