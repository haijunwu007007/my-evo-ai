...
window.history.replaceState({{}},"","/")}})</script></body>"""
        ...
        # /workflow 路由由 routes_static.py 处理（含 _WORKFLOW_HTML 导入）
...
async def lifespan(app: FastAPI):
    ...
    # 追加路由到 app.routes 末尾（Starlette反转匹配，末尾路由优先匹配）
    try:
        from starlette.routing import Route
        from fastapi.responses import FileResponse, JSONResponse
        async def _ping_handler(_request):
            return JSONResponse({"ok": True})
        app.routes.append(Route("/__ping__", endpoint=_ping_handler))
    except Exception as e:
        logger.warning(f"[ROUTES-APPEND] error: {e}")
