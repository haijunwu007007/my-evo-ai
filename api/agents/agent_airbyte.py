...
class AirbyteIntegration:
    ...
    def _status(self, **kw) -> dict:
        return {"ok": True, "project": self.name, "capability": self.capability,
                "status": "ready", "note": "ETL数据采集/清洗/同步管道，需配置AIRBYTE_URL环境变量"}
...
def airbyte_sync(**kwargs) -> dict:
    """ETL数据管道"""
    try:
        integration = AirbyteIntegration()
        action = kwargs.pop("action", "status")
        return integration.execute(action, **kwargs)
    except Exception as e:
        log.error(f"airbyte_sync error: {e}")
